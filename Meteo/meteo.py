...
class Meteo:
    ...
    def update(self):
        ...
        table_meteo = re.findall(
            r"<table style=\"border-collapse: collapse;\" \s?border=1 \s?cellpadding=2 \s?cellspacing=0 \s?bordercolor=#a0a0b0\s?>.*</table>",
            page.decode("iso-8859-1"),
        )
    ...
    def generateImage(self):
        ...
        # if icon:
        iconDeg = Image.open(
            os.path.join(os.path.dirname(__file__), "./icons/degC.png")
        )
        im.paste(iconDeg, (12, 8), iconDeg.convert("RGBA"))
        iconThermo = Image.open(
            os.path.join(os.path.dirname(__file__), "./icons/thermo.png")
        )
        im.paste(iconThermo, (1, 8), iconThermo.convert("RGBA"))
        iconPeunuageux = Image.open(
            os.path.join(
                os.path.dirname(__file__),
                "./icons/soleil" + str(self.sequence % 2) + ".png",
            )
        )
        im.paste(iconPeunuageux, (0, 0), iconPeunuageux.convert("RGBA"))
        font = ImageFont.truetype(
            os.path.join(os.path.dirname(__file__), "../src/pixelated.ttf"), size=8
        )
