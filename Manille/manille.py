...
class Manille:
    ...
    def generateImage(self, score1=0, score2=0):
        ...
        for i in range(8):
            current_color = (
                int(color1[0] + ((color2[0] - color1[0]) / 8 * i)),
                int(color1[1] + ((color2[1] - color1[1]) / 8 * i)),
                int(color1[2] + ((color2[2] - color1[2]) / 8 * i)),
            )
            shape = [(0, i), (15, i)]
        ...
        font = ImageFont.truetype(
            os.path.join(os.path.dirname(__file__), "../src/pixelmix.ttf"), size=8
        )
